...
void LocalMemUsageTracker::recordReads(
    ...
    const Chakra::FeederV3::DependancyResolver& depResolver =
        this->feeder->getDependancyResolver();
    const std::unordered_set<Chakra::FeederV3::NodeId>& parents =
        depResolver.get_data_dependancy().get_parents(node->id());
    for (const NodeId parent : parents) {
        std::shared_ptr<Chakra::FeederV3::ETFeederNode> parentNode =
            this->feeder->lookupNode(parent);
        TensorId tensorName = parentNode->name();
        uint64_t tensorSize =
            parentNode->get_attr<uint64_t>("y_tensor_size", 0);
...
void LocalMemUsageTracker::recordWrites(
    ...
    for (int64_t i = 0; i < 1; i++) {
        TensorId tensorName = node->name();
        uint64_t tensorSize = node->get_attr<uint64_t>("y_tensor_size", 0);
